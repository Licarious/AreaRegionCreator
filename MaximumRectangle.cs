namespace AreaRegionCreator
{
    public class MaximumRectangle
    {
        ...
        public (int area, int top, int bottem, int left, int right) maxRectangle(int R, int C, int[][] A, double w = 1) {
            int top = 0;
            int bottem = 0;
            ...
            for (int i = 1; i < R; i++) {
                ...
                // Calculate the area of the current rectangle using the weight value
                int tmp_area = (int)(w * (tmp_right - tmp_left + 1) * (i - (bottem - tmp_result / (tmp_right - tmp_left + 1) + 1)));
        ...
        public ((int, int), (int, int)) center(List<(int, int)> coordList, bool floodFill, bool useMaxSquare = false, double widthWeight = 1.5) {
            (int, int) center = (0, 0);
            (int, int) size = (0, 0);
            ...
            //find the largest rectangle without holes in the region and return the top left and bottom right corners of the rectangle as a tuple
            (int area, int top, int bottem, int left, int right) = maxRectangle(maxX - minX + 1, maxY - minY + 1, A, widthWeight);
        ...
        public ((int x, int y), (int height, int width)) centerWeightedWidth(List<(int, int)> coordList, bool floodFill, double widthWeight) {
            // Initialize variables to track the maximum rectangle found so far
            int maxX = 0;
            int maxY = 0;
            int maxWidth = 0;
            int maxHeight = 0;
            ...
            foreach ((int x, int y) in coordList) {
                // Loop through each other point in the list
                foreach ((int otherX, int otherY) in coordList) {
                    // Calculate the width and height of the rectangle formed by these two points
                    int width = Math.Abs(x - otherX);
                    int height = Math.Abs(y - otherY);

                    // Check if this rectangle includes any points that are not in the list
                    bool valid = true;
                    for (int i = Math.Min(x, otherX); i <= Math.Max(x, otherX); i++) {
                        for (int j = Math.Min(y, otherY); j <= Math.Max(y, otherY); j++) {
                    ...
                    if (valid) {
                        // Calculate the priority for this rectangle (1.5x weight on width)
                        int priority = (int)(widthWeight * width + height);

                        // Update the maximum rectangle if this rectangle has a higher priority
                        if (priority > maxWidth + maxHeight) {
                            maxX = Math.Min(x, otherX);
                            maxY = Math.Min(y, otherY);
                            maxWidth = width;
                            maxHeight = height;
                        }
                    }
            ...
            // Calculate the center point of the largest rectangle
            int centerX = maxX + maxWidth / 2;
            int centerY = maxY + maxHeight / 2;

            // Print the center point and size of the largest rectangle
            Console.WriteLine("Center: (" + centerX + ", " + centerY + ")");
            Console.WriteLine("Size: " + maxWidth + " x " + maxHeight);

            return ((centerX, centerY), (maxHeight, maxWidth));
        }

        public ((int x, int y), (int height, int width)) GetLargestRectangle(List<(int x, int y)> points, bool floodFill, double widthWeight = 1) {
            ...
            for (int i = 0; i < points.Count; i++) {
                ...
                for (int j = i + 1; j < points.Count; j++) {
                    // Calculate the width and height of the rectangle formed by these two points
                    int width = Math.Abs(points[i].x - points[j].x);
                    int height = 0;
                    ...
                    // Update the maximum rectangle if this rectangle has a higher priority
                    if (priority > maxWidth + maxHeight) {
                        maxX = Math.Min(points[i].x, points[j].x);
                        maxY = Math.Min(points[i].y, points[j].y);
                        maxWidth = width;
                        maxHeight = height;
                    }
                }
